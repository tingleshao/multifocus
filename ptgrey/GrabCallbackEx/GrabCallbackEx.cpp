...
void OnImageGrabbed(Image* pImage, const void* pCallbackData)
{
      ...
      //  {
           cout << "hi" << endl;
           unsigned int rowBytes = (double)pImage->GetReceivedDataSize()/(double)pImage->GetRows();
           cv::Mat mat = cv::Mat( pImage->GetRows(), pImage->GetCols(), CV_8UC1, pImage->GetData(), rowBytes);
          // mat = cv::cvarrToMat(ConvertImageToOpenCV(pImage));
           cv::imshow("CV_Image0", mat);
           cv::waitKey(0);	
//        }
//	cout << "Grabbed image " << imageCnt++ << endl;
        cout << "Grabbed iamge " << 0 << endl;
 	return;
}
